    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       if (pathInfo != null && pathInfo.startsWith("/delete/")) {
            try {
                ...
                CustomerDAO dao = new CustomerDAO();
                // Gọi phương thức deleteAccount để cập nhật Role=2 thay vì xóa
                if (dao.deleteAccount(customerId)) {
                    request.getSession().setAttribute("message", "Đã đánh dấu tài khoản khách hàng là hết hạn thành công!");
                    request.getSession().setAttribute("messageType", "success");
                    response.sendRedirect(request.getContextPath() + "/CustomerManagerAd");
                } else {
                    request.getSession().setAttribute("message", "Không thể đánh dấu tài khoản khách hàng là hết hạn!");
                    request.getSession().setAttribute("messageType", "error");
                    response.sendRedirect(request.getContextPath() + "/CustomerManagerAd");
                }
            } catch (SQLException | NumberFormatException e) {
                Logger.getLogger(EditCustomerServlet.class.getName()).log(Level.SEVERE, null, e);
                request.getSession().setAttribute("message", "Lỗi: ID khách hàng không hợp lệ hoặc lỗi hệ thống!");
                request.getSession().setAttribute("messageType", "error");
                response.sendRedirect(request.getContextPath() + "/CustomerManagerAd");
            } catch (ClassNotFoundException ex) {
                Logger.getLogger(EditCustomerServlet.class.getName()).log(Level.SEVERE, null, ex);
                request.getSession().setAttribute("message", "Lỗi hệ thống khi xử lý yêu cầu!");
                request.getSession().setAttribute("messageType", "error");
                response.sendRedirect(request.getContextPath() + "/CustomerManagerAd");
            }
        }else {
