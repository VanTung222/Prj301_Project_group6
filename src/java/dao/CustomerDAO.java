...
public class CustomerDAO {
   ...
   private static final String UPDATE_ROLE_TO_EXPIRED = "UPDATE Customers SET Role=2 WHERE Customer_ID=?";
  ...
  public boolean deleteAccount(int customerId) throws SQLException, ClassNotFoundException {
        ...
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                ps = conn.prepareStatement(UPDATE_ROLE_TO_EXPIRED);
                ps.setInt(1, customerId);
...
//    }
    ...
    public List<Customer> getCustomersByPage(int offset, int pageSize) throws SQLException {
        List<Customer> customerList = new ArrayList<>();
        String sql = "SELECT Customer_ID, GoogleID, Email, Username, FirstName, LastName, Password, " +
                     "ProfilePicture, Address, Phone, Registration_Date, Role " +
                     "FROM Customers " +
                     "WHERE Role IN (0, 1) " + // Chỉ lấy Admin (0) và Customer (1)
    ...
    // Lấy khách hàng theo Role với phân trang
    public List<Customer> getCustomersByRoleAndPage(int role, int offset, int pageSize) throws SQLException {
        List<Customer> customerList = new ArrayList<>();
        String sql = "SELECT Customer_ID, GoogleID, Email, Username, FirstName, LastName, Password, " +
                     "ProfilePicture, Address, Phone, Registration_Date, Role " +
                     "FROM Customers " +
    ...
    // Lấy tổng số khách hàng đang hoạt động (Role=0 hoặc Role=1)
    public int getTotalActiveCustomers() throws SQLException, ClassNotFoundException {
        String sql = "SELECT COUNT(*) as count FROM Customers WHERE Role IN (0, 1)";
        try (Connection conn = DBUtils.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
    ...
    // Lấy tổng số khách hàng theo Role cụ thể
    public int getTotalCustomersByRole(int role) throws SQLException, ClassNotFoundException {
        String sql = "SELECT COUNT(*) as count FROM Customers WHERE Role = ?";
        try (Connection conn = DBUtils.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, role);
