...
public class CustomerManagerDAO {
    ...
    public List<Customer> getCustomersByRole(boolean role) throws SQLException {
        ...
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                String sql = "SELECT Customer_ID, GoogleID, Email, Username, FirstName, LastName, Password, ProfilePicture, Address, Phone, Registration_Date, Role FROM Customers WHERE Role==1";
                ps = conn.prepareStatement(sql);
